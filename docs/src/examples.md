...
p1 = heatmap(true_sky, title="True Sky", cmap=:jet, aspect_ratio=:equal, cbar=false)
p2 = heatmap(images[:, :, 1], title="Single Frame", cmap=:jet, aspect_ratio=:equal, cbar=false)
p3 = heatmap(
    mean(images, dims=3)[:, :, 1],
    title="Average",
    colormap=:jet,
